...
public class LreWindowSelector {
     ...
     * processed
     */
    public static void selectLreStartCycleViaScanning(ProfileSummary profileSummary, double foThreshold) {
        ...
        scanForAdjustedStartCycle(profileSummary, foThreshold);
     ...
     * must be >0
     */
    public static void selectLreStartCycleUsingMinFc(ProfileSummary prfSum, double minFc, double foThreshold) {
//A warning dialog will be shown if the minFc is possible too large or too small
        ...
        if (!profile.hasAnLreWindowBeenFound()) {
            selectLreStartCycleViaScanning(prfSum, foThreshold);
            if (!profile.hasAnLreWindowBeenFound()) {
...
//Failed Profile or the window is at the end of the profile and thus cannot be expanded
    ...
    private static void scanForAdjustedStartCycle(ProfileSummary profileSummary, double foThreshold) {
        ProfileSummary tempProfileSummary = profileSummary.cloneSummary();
        Profile realProfile = profileSummary.getProfile();
        Profile tempProfile = tempProfileSummary.getProfile();
        while (tempProfile.getStartingCycleIndex() > 0) {
            int originalStartIndex = tempProfile.getStartingCycleIndex();
            tempProfile.setStartingCycleIndex(originalStartIndex - 1);
            tempProfile.setLreWinSize(tempProfile.getLreWinSize() + 1);
            tempProfileSummary.update();

            // If the F0 value generated by the cycle below the current start cycle
            // is within the F0 threshold of the current average F0 then include it.
            // Otherwise stop adjusting the start cycle.
            Cycle cycleToInclude = tempProfileSummary.getLreWindowStartCycle();
            if (Math.abs(cycleToInclude.getFoFracFoAv()) < foThreshold) {
                realProfile.setStartingCycleIndex(originalStartIndex - 1);
                profileSummary.update();
            }
            else {
                break;
